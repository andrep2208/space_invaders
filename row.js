  ...
  this.hit = function (shootup) {
    for (var j = 0; j < row.enemy.length ; j++) {
      for (var i = 0; i <shootup.length ; i++) {
        if (shootup[i].x -7.5 < row.enemy[j].x + 80 + row.enemy[j].speed && shootup[i].x +7.5 > row.enemy[j].x + row.enemy[j].speed  &&  
            shootup[i].y < row.enemy[j].y +20 && shootup[i].y > row.enemy[j].y ) {
          //console.log("hit");
